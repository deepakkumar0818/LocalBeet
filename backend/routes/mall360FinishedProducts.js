...
// POST /api/mall360/finished-products/import - Import finished products via JSON array
  ...
  try {
    ...
    for (let i = 0; i < products.length; i++) {
      try {
        ...
        const existing = await Mall360FinishedProduct.findOne({ productCode: productData.productCode });
        if (existing) {
          const updateData = {
            ...
            // Ensure both category and subCategory are set; category maps to SubCategory Name in sheet
          ...
          await Mall360FinishedProduct.findByIdAndUpdate(existing._id, updateData, { new: true, runValidators: true });
          successCount++;
        } else {
          const newProduct = {
          ...
          await Mall360FinishedProduct.create(newProduct);
          successCount++;
        }
      } catch (e) {
    ...
    console.error('Error importing 360 Mall finished products:', error);
