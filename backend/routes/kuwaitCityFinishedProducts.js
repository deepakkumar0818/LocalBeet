...
// POST /api/kuwait-city/finished-products/import - Import finished products via JSON array
router.post('/import', async (req, res) => {
  try {
    ...
    // Helper to normalize status to model enum
    const normalizeStatus = (rawStatus, currentStock, reorderPoint) => {
      ...
      // Derive from numbers if not mapped
    ...
    for (let i = 0; i < products.length; i++) {
      try {
        ...
        const existing = await KuwaitCityFinishedProduct.findOne({ productCode: productData.productCode });
        if (existing) {
          const updateData = {
            ...
            // Map subCategory/category consistently
          ...
          await KuwaitCityFinishedProduct.findByIdAndUpdate(existing._id, updateData, { new: true, runValidators: true });
          successCount++;
        } else {
          const newProduct = {
          ...
          await KuwaitCityFinishedProduct.create(newProduct);
          successCount++;
        }
      } catch (e) {
    ...
    console.error('Error importing Kuwait City finished products:', error);
