...
const resetAndSeedAllDataSingle = async () => {
  console.log('🚀 Starting complete data reset and seeding process (Single Script)...\n');
  ...
  try {
    ...
    console.log('\n' + '='.repeat(60));
    console.log('STEP 2: SEEDING RAW MATERIALS DATA');
    console.log('='.repeat(60));
    
    const seedRawMaterials = async (models, outletName) => {
      console.log(`🌱 Seeding ${outletName} Raw Materials...`);
      for (const data of rawMaterialsData) {
        const rawMaterialData = {
          ...
          shelfLife: 365,
          isActive: true,
          status: outletName === 'Central Kitchen' ? 'Active' : 'In Stock',
          notes: 'Fresh batch received',
          createdBy: 'System',
          updatedBy: 'System'
        };
        
        if (outletName === 'Central Kitchen') {
          await models.centralKitchen.CentralKitchenRawMaterial.create(rawMaterialData);
        } else if (outletName === 'Kuwait City') {
        ...
        } else if (outletName === 'Vibe Complex') {
    ...
    await seedRawMaterials(models, 'Central Kitchen');
    await seedRawMaterials(models, 'Kuwait City');
    await seedRawMaterials(models, '360 Mall');
    await seedRawMaterials(models, 'Vibe Complex');

    // Step 3: Seed Finished Goods
    console.log('\n' + '='.repeat(60));
    console.log('STEP 3: SEEDING FINISHED GOODS DATA');
    console.log('='.repeat(60));
    
    const seedFinishedGoods = async (models, outletName) => {
      console.log(`🌱 Seeding ${outletName} Finished Goods...`);
    ...
    await seedFinishedGoods(models, 'Central Kitchen');
    await seedFinishedGoods(models, 'Kuwait City');
    await seedFinishedGoods(models, '360 Mall');
    await seedFinishedGoods(models, 'Vibe Complex');
    ...
    console.error('❌ Error during complete data reset and seeding:', error);
    process.exit(1);
  } finally {
...
module.exports = { resetAndSeedAllDataSingle };
