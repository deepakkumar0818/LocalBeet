...
import { Package, TrendingUp, TrendingDown, AlertTriangle, BarChart3, Truck, Users, Clock, RefreshCw, ShoppingBag, Download, Upload, ShoppingCart, Search } from 'lucide-react'
...
const MallFoodCourt: React.FC = () => {
    ...
    try {
      ...
      if (outletsResponse.success) {
        ...
        if (mallFoodCourt) {
          ...
          await loadInventory(mallFoodCourt._id)
        } else {
  ...
  const loadInventory = async (outletId?: string) => {
    const currentOutletId = outletId || outlet?._id || 'mall-food-court-001'
    ...
    try {
      ...
      console.log('Loaded Vibes Complex Inventory:', sampleRawMaterials)
  ...
  const handleExport = () => {
    ...
    link.setAttribute('download', `vibes-complex-raw-materials-${new Date().toISOString().split('T')[0]}.csv`)
  ...
  const downloadSampleCSV = () => {
    ...
    const url = URL.createObjectURL(blob)
    link.setAttribute('href', url)
    link.setAttribute('download', 'vibes-complex-raw-materials-sample.csv')
    link.style.visibility = 'hidden'
    document.body.appendChild(link)
    link.click()
    document.body.removeChild(link)
