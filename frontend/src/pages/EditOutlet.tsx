...
const EditOutlet: React.FC = () => {
  ...
  const loadOutlet = async () => {
    try {
      ...
      const response = await apiService.getOutlet(id!)
      if (response.success) {
        setFormData(response.data)
      } else {
        alert('Failed to load outlet')
        navigate('/outlets')
      }
    } catch (error) {
      alert('Error loading outlet: ' + (error instanceof Error ? error.message : 'Unknown error'))
      navigate('/outlets')
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      const response = await apiService.updateOutlet(id!, formData)
      if (response.success) {
      ...
      alert('Error updating outlet: ' + (error instanceof Error ? error.message : 'Unknown error'))
