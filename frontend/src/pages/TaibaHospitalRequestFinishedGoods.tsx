...
const TaibaHospitalRequestFinishedGoods: React.FC = () => {
  const navigate = useNavigate()
  const [finishedGoods, setFinishedGoods] = useState<any[]>([])
  const [formData, setFormData] = useState({
    transferNumber: '',
    fromWarehouseId: 'Taiba Hospital',
    fromWarehouseName: 'Taiba Hospital',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const transferOrderData = {
        ...
        requestedBy: 'Taiba Hospital Manager'
      }
      ...
      if (response.success) {
        ...
        navigate('/drive-thru-express/finished-goods')
      } else {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          <button
            onClick={() => navigate('/drive-thru-express/finished-goods')}
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Transfer Number *</label>
                <input
                  type="text"
              ...
              <div>
                ...
                <select className="input-field" value={formData.transferType} onChange={(e) => handleInputChange('transferType', e.target.value)}>
                  {transferTypeOptions.map(type => <option key={type} value={type}>{type}</option>)}
                </select>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Priority</label>
                <select className="input-field" value={formData.priority} onChange={(e) => handleInputChange('priority', e.target.value)}>
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-medium text-gray-900">Finished Goods ({finishedGoods.length} available)</h3>
              <button type="button" onClick={addTransferOrderItem} className="btn-secondary flex items-center text-sm">
            ...
            {formData.items.length === 0 ? (
              ...
              <div className="space-y-4">
                {formData.items.map((item, index) => (
                  <div key={index} className="grid grid-cols-7 gap-3 p-4 border border-gray-200 rounded-lg bg-green-50">
                    <div>
                      <label className="block text-xs font-medium text-gray-700 mb-1">Product Code</label>
                      <select className="input-field text-sm" value={item.materialCode} onChange={(e) => handleProductCodeChange(index, e.target.value)}>
                    ...
                    <div>
                      ...
                      <input type="text" className="input-field text-sm bg-gray-100" value={item.materialName} placeholder="Product Name" readOnly />
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-gray-700 mb-1">Quantity</label>
                      <input type="number" className="input-field text-sm" value={item.quantity} onChange={(e) => updateTransferOrderItem(index, 'quantity', parseFloat(e.target.value) || 0)} placeholder="0" />
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-gray-700 mb-1">Unit</label>
                      <input type="text" className="input-field text-sm bg-gray-100" value={item.unitOfMeasure} placeholder="Unit" readOnly />
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-gray-700 mb-1">Unit Price</label>
                      <input type="number" step="0.01" className="input-field text-sm bg-gray-100" value={item.unitPrice} placeholder="0.00" readOnly />
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-gray-700 mb-1">Remarks</label>
                      <input type="text" className="input-field text-sm" value={item.remarks || ''} onChange={(e) => updateTransferOrderItem(index, 'remarks', e.target.value)} placeholder="Notes" />
                    </div>
                    <div className="flex items-end">
                      <button type="button" onClick={() => removeTransferOrderItem(index)} className="text-red-600 hover:text-red-900 p-1">
                        <Trash2 className="h-4 w-4" />
                      </button>
                    </div>
                    <div className="col-span-7">
                      <div className="text-sm text-gray-600">Total Price: {item.totalPrice.toFixed(2)} KWD</div>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <textarea className="input-field" rows={3} value={formData.notes} onChange={(e) => handleInputChange('notes', e.target.value)} placeholder="Additional notes or special instructions" />
              </div>
            </div>
          </div>

          <div className="flex justify-end space-x-4 pt-6 border-t">
            <button type="button" onClick={() => navigate('/drive-thru-express/finished-goods')} className="btn-secondary flex items-center">
...
export default TaibaHospitalRequestFinishedGoods
